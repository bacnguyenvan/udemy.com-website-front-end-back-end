$(document).ready(function(){
	...
	//filter
	$('.filter_content li a').click(function(e){
		e.preventDefault();
		var course = $(this).data('class');
		$('.filter_content li').removeClass('active_filter');
		$(this).parent().addClass('active_filter');

	});

	//recommend course message
	$('.btn_ok').click(function(){
		

		$.ajax({
			type: "post",
        	data:  $('#form_goi_y').serialize(),
          	url: "ajax_timkiem.php"
          }).done(function(data) {
          	$('#carouselExampleControls1 .carousel-inner').html(data);
	...
	// click choose type of course
	$('.filter_content li a').click(function(e){
		e.preventDefault();
		$type_of_course = $(this).attr('data-id');
		if(!$type_of_course) $type_of_course=0;
		$.ajax({
			url:'ajax_click.php?id_typeOfcourse='+$type_of_course,
